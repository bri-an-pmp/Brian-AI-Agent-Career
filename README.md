...
Built with Gradio (https://www.gradio.app/) for the chat UI, deployed on Hugging Face Spaces. Uses the OpenAI API (gpt-4o-mini) to power the conversation, with a system prompt grounded on a LinkedIn profile PDF (me/linkedin.pdf) and a written career summary (me/summary.txt).
...
Python, Gradio, the OpenAI API (openai, openai-agents), pypdf for parsing the LinkedIn PDF, Pushover for notifications, and python-dotenv for local environment config.

## Project Structure

app.py is the main Gradio app and agent logic. requirements.txt lists the Python dependencies. The me/ folder holds linkedin.pdf (exported LinkedIn profile used as agent context) and summary.txt (career summary used as agent context).

## Setup

Clone the repo and install dependencies with pip install -r requirements.txt. Create a .env file with OPENAI_API_KEY, PUSHOVER_TOKEN, and PUSHOVER_USER. Add your own career materials to a me/ folder: me/linkedin.pdf (your exported LinkedIn profile) and me/summary.txt (a short written summary of your background). Then run locally with python app.py.
